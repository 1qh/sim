...
const registerKmap = (c: string): void => {
...
for (const c of KMAP_CASES) registerKmap(c)
const registerPipeline = (p: string): void => {
...
for (const p of PIPELINE_PROGRAMS) registerPipeline(p)
